(function(){ 
angular.module('Dashboard', ['ui.bootstrap', 'ui.router', 'ngCookies','nvd3ChartDirectives']);
'use strict';
...
angular.module('Dashboard')
    ...
    .controller('ScoreChartCtrl', ['$scope', ScoreChartCtrl])
...
function ScoreChartCtrl($scope) {
    $scope.scoreChart = [       
            {"key" :"HIGH","y":199},
            {"key" :"LOW","y":83},
            {"key" :"MEDIUM","y":268}
        ];
    /*$http.jsonp('http://54.225.232.25:3000/score' + "?callback=JSON_CALLBACK").
      success(function(data, status, headers, config){
        console.log(data.found);
        alert("str(data)");
            $scope.scoreChart = function(data){
                var arr = [];
                var len = data.length;
                for (var i = 0; i < len; i++) {
                    arr.push({
                        key : str(data[i].Score),
                        y : data[i].Total
                    });
                }
                alert(str(arr));
