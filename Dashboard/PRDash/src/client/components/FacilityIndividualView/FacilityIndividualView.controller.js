...
angular.module('perceptiveReachApp')
  .controller('FacilityIndividualViewCtrl', function ($scope,$http,$rootScope) {
        ...
        var VAMC_Id = $scope.facilitySelected; //VA Medical Facility ID
        ...
        $scope.backToStateView = function(){
            $rootScope.hideStateView = false;
            $rootScope.hideFacilityView = true;
        }
        $scope.backToFacilityView = function(){
            $rootScope.hideIndividualView = true;
            $rootScope.hideFacilityView = false;
        }
        $scope.viewVeteranDetails = function(){
            $rootScope.hideIndividualView = false;
            $rootScope.hideFacilityView = true;
        }
  ...
  .directive('dataTablesVet', function(){
    var linker = function(scope,element, attr){
        var unwatch = scope.$watch('dataSetVet', function(v){
            if(v){
                unwatch();
                var dataTableVet = $(element).dataTable( {
                    "data": scope.dataSetVet,
                    "scrollY":        "200px",
                    "scrollCollapse": true,
                    "paging":         false,
                    "columns": [
