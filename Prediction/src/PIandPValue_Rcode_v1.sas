...
proc IML;
  ...
  rdata1<-read.csv(file)
  attach(rdata1)
  names(rdata1)
  ##rdata2<-rdata1[,c(1,2,3,4,5,6,10,11,13,14,15,28,29,30)]
  ##attach(rdata2)
  rdata3<-rdata1[15:17,]
  names(rdata3)
  rdata3
...
## Read data file
file<-"C:\\Surveillance_partIII\\RInputData\\plot_c_R.csv"
rdata3<-readData(file)
alpha<-0.1
beta<-(1-(alpha/2))
Index<-1
IMth<-15
...
for (Index in 1:3){
  ...
  ##colnames(df)<-c("Obs","month","y","pdf","cdf","pvalue","lcl","ucl")
  ##print(df)
  ##colnames(rdata4)<-c("Obs","month","y","pdf","cdf","pvalue","lcl","ucl")
  ##print(rdata4)
  IMth<-IMth+1
}
...
rdata4<-rdata4[,1:9]
print(rdata4)
#write.csv(rdata4, file = "C:\\Surveillance_partI\\ROutputData\\fac39.csv",row.names=FALSE)
ENDSUBMIT;
RUN ImportDataSetFromR("Work.plot_c_sas","rdata4");
QUIT;
